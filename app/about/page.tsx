...
export default function About() {
  ...
  return (
    <div className="bg-white p-10 text-gray-800">
      <Head>
        ...
        <meta name="description" content="Learn more about Gia Jung, a Computational Designer at Google." />
  ...
  <h1 className="text-3xl font-semibold text-gray-800 mb-4">{translations[language].title}</h1>
  {(translations[language].about as string[]).map((paragraph, index) => (
    <p key={index} className="text-base text-gray-700 leading-relaxed mb-4">
      ...
      <footer className="absolute bottom-4 text-center w-full m-0">
        <p className="text-sm">
          ...
          <a href="https://www.linkedin.com/in/giajung/" target="_blank" rel="noopener noreferrer" className="hover:text-gray-600">{translations[language].name}</a>
        </p>
      </footer>
    </div>
  );
