...
const languageAbbreviations: string[] = Array.from(new Set(Object.values(supportedLanguages))).sort();
...
const mapLanguage = (languageCode: string | null): string => {
  ...
  const code = languageCode.toLowerCase().split('-')?.[0] as keyof typeof supportedLanguages;
    ...
    <div className="float-right fixed top-0 right-0 p-4">
      {languageAbbreviations.map((lang, i) => {
        return (
            <><button
            key={lang}
            className={`hover:text-gray-600 ${language === lang ? 'font-bold' : ''}`}
            onClick={() => handleSetLanguage(lang)}
            >
            {lang}
            </button>
            {i < languageAbbreviations.length - 1 && <span className="mx-2" key={`${lang}-separator`}>|</span>}
            </>
        );
        })}
    </div>
